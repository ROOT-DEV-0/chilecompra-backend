...
app.post('/api/setup', async (req, res) => {
  ...
  const user = {
    ...
    name: name || 'Admin', password: await bcrypt.hash(password, 12),
...
app.post('/api/admin/users', admin, async (req, res) => {
  ...
  const user = {
    id: Date.now().toString(), email: email.toLowerCase(),
    name: name || email.split('@')[0], password: await bcrypt.hash(password, 12),
...
app.post('/api/alerts', auth, (req, res) => {
  ...
  const a = {
    ...
    nombre: req.body.nombre || 'Sin nombre', keywords: req.body.keywords || [],
    tipos: req.body.tipos || [], estados: req.body.estados || ['5'],
...
async function runPolling() {
  ...
  try {
    ...
    for (const alerta of alerts) {
      ...
      if (idx >= 0) {
        saved[idx].matchCount = (saved[idx].matchCount || 0) + nuevas.length
        saved[idx].ultimaVez = new Date().toISOString()
        writeJ(F.alerts, saved)
      }
